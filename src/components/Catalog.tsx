...
import { ShoppingBag, User, Phone, MapPin, Notebook, CheckCircle, Trash2, Plus, Minus, Search, SlidersHorizontal, X, ArrowUpDown } from "lucide-react";
  ...
  useEffect(() => {
    const loadCatalogData = async () => {
      try {
        ...
        if (cachedData && cacheTime && (Date.now() - Number(cacheTime)) < 24 * 60 * 60 * 1000) {
          const parsed = JSON.parse(cachedData);
          setProducts([...parsed.catalog].sort(() => Math.random() - 0.5));
          setCategories(Array.from(new Set(["সব", ...parsed.catalog.flatMap((p: any) => p.Category)])));
          setIsLoading(false);
          // Still fetch in background to refresh? Maybe not if user wants local file control
        }
    ...
    <section ref={catalogRef} id="catalog" className="py-20 px-1 sm:px-10 w-full min-h-[1000px] sm:min-h-screen">
      <div className="flex flex-col mb-16 max-w-7xl mx-auto px-4 sm:px-0">
        ...
        <div className="flex flex-col items-center text-center gap-6">
          <h2 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-black font-display leading-[1.3] md:leading-[1.1] text-white tracking-tighter">
            সেরা পণ্যগুলো <br /> 
        ...
        {isFilterPanelOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-lg max-h-[90vh] bg-slate-900 border border-white/10 rounded-[2rem] overflow-y-auto no-scrollbar shadow-[0_0_100px_rgba(0,0,0,0.5)]"
            >
              <div className="p-6 md:p-8">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <button 
                    onClick={() => setIsFilterPanelOpen(false)}
                    className="w-10 h-10 bg-white/5 border border-white/10 rounded-xl flex items-center justify-center text-white/30 hover:text-white transition-all"
                  >
                    <X size={20} />
                  </button>
                </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    <span className="text-[9px] uppercase font-black tracking-[0.2em] text-white/40">পণ্য খুঁজুন</span>
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full bg-black/40 border border-white/10 rounded-xl pl-12 pr-6 py-3.5 text-white placeholder:text-white/20 focus:outline-none focus:border-neon-blue transition-all"
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <span className="text-[9px] uppercase font-black tracking-[0.2em] text-white/40">মূল্যর পরিসীমা</span>
                      <div className="flex items-center gap-1.5">
                         <span className="text-[10px] font-bold text-white bg-white/5 px-2 py-1 rounded border border-white/5">{tempMin}৳</span>
                         <span className="text-white/20">-</span>
                         <span className="text-[10px] font-bold text-white bg-white/5 px-2 py-1 rounded border border-white/5">{tempMax}৳</span>
                      </div>
                    </div>
                  ...
                  {/* Categories */}
                  <div className="space-y-3">
                    <span className="text-[9px] uppercase font-black tracking-[0.2em] text-white/40">ক্যাটাগরি</span>
                ...
                <div className="flex gap-3 mt-8 pt-6 border-t border-white/5">
                  <button 
                    ...
                    className="flex-1 bg-white/5 text-white/40 font-bold py-3.5 rounded-xl hover:bg-white/10 transition-all text-[10px] uppercase tracking-widest"
                  >
                    রিসেট
                  </button>
                  <button 
                    onClick={applyFilters}
                    className="flex-[2] bg-neon-blue text-black font-black py-3.5 rounded-xl shadow-lg shadow-neon-blue/20 transition-transform active:scale-95 text-[10px] uppercase tracking-widest"
                  >
                    ফিল্টার APPLY
                  </button>
                </div>
      ...
      <AnimatePresence>
        {isSticky && (
          <motion.div
            ...
            exit={{ y: 50, opacity: 0 }}
            className="fixed bottom-6 md:bottom-auto md:top-[104px] left-0 right-0 z-[100] px-4"
          >
            <div className="max-w-xl mx-auto bg-slate-900/90 backdrop-blur-3xl p-1.5 rounded-[2rem] flex items-center justify-center shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/10">
              <div className="flex flex-wrap gap-1 md:gap-2 justify-center w-full px-1">
                {categories.slice(0, 8).map((cat) => (
                  <button 
                    ...
                    className={cn(
                      "px-3 md:px-5 py-2 md:py-3 rounded-2xl text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap",
                ...
                {categories.length > 8 && (
                  <button 
                    onClick={() => {
                      ...
                      // Scroll slightly to trigger filter if needed, but opening modal is enough
                    }}
                    className="px-3 md:px-5 py-2 md:py-3 rounded-2xl text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap text-white/40 hover:text-white hover:bg-white/5"
                  >
                    ...
                  </button>
                )}
        ...
        <AnimatePresence>
          {isLoading ? (
            <div className="col-span-full py-20 text-center">
              ...
              <p className="mt-4 text-white/40 font-bold uppercase tracking-widest text-[10px]">পণ্য লোড হচ্ছে...</p>
            </div>
            ...
            >
              ...
              <h3 className="text-xl font-bold text-white/40">কোনো ফলাফল পাওয়া যায়নি</h3>
              <p className="text-white/20 text-sm">অনুগ্রহ করে ভিন্ন কিছু খুঁজুন বা ফিল্টার রিসেট করুন।</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
      ...
      <AnimatePresence>
        {cart.length > 0 && (
          <motion.div 
            ...
            viewport={{ once: true }}
            id="checkout-panel" 
            className="mt-20 md:mt-40 bg-black/60 backdrop-blur-[60px] p-6 sm:p-8 md:p-16 rounded-[2.5rem] md:rounded-[4rem] border border-white/20 relative overflow-hidden shadow-[0_0_100px_rgba(0,242,255,0.1)]"
          >
            ...
            <div className="max-w-6xl mx-auto relative z-10">
              <div className="text-center mb-16">
                ...
                <motion.h3 
                  ...
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }}
                  className="text-4xl md:text-6xl font-black font-display text-white mb-6 tracking-tight"
                >
                  আপনার অর্ডার ফাইনাল করুন
                </motion.h3>

                <p className="text-white/60 text-lg max-w-2xl mx-auto">সঠিক তথ্য প্রদান করে আপনার পছন্দের পণ্যগুলো আজই ঘরে পৌঁছে নিন।</p>
              ...
              <div className="flex flex-col lg:flex-row gap-12">
                ...
                <div className="flex-1 space-y-8">
                  <div className="flex items-center gap-3 text-neon-blue mb-6">
                    <ShoppingBag size={24} />
                    <h4 className="text-2xl font-bold text-white">আপনার পণ্যসমূহ ({cart.length})</h4>
                  </div>
                      ...
                      {cart.length > 0 ? (
                        cart.map((item) => (
                          <motion.div 
                            ...
                            exit={{ opacity: 0, scale: 0.9 }}
                            className="bg-white/5 border border-white/10 rounded-2xl sm:rounded-3xl p-3 sm:p-4 flex items-center gap-3 sm:gap-6 group hover:border-white/30 transition-all shadow-lg backdrop-blur-md"
                          >
                            <div className="w-16 h-16 sm:w-20 sm:h-20 bg-slate-800 rounded-xl sm:rounded-2xl flex items-center justify-center p-2 border border-white/5 shrink-0">
                              <img src={item.image} alt={item.title} className="w-full h-full object-contain" />
                            </div>
                            <div className="flex-1 min-w-0">
                              <h5 className="text-white font-bold text-sm sm:text-lg mb-0.5 sm:mb-1 truncate leading-[1.6]">{item.title}</h5>
                              <div className="flex flex-col">
                                <div className="flex items-center gap-2">
                                  <p className="text-neon-blue font-bold text-xs sm:text-base tracking-tight leading-[1.6]">{item.price}</p>
                                  {item.size && (
                                    <span className="text-[8px] sm:text-[10px] bg-white/10 px-1.5 sm:px-2 py-0.5 rounded text-white/60 font-bold uppercase tracking-widest">{item.size}</span>
                                  )}
                                </div>
                                <p className="text-[9px] sm:text-[10px] text-white/40 font-bold mt-1 leading-[1.6]">
                                  {item.priceNum} x {item.quantity} = {(item.priceNum * item.quantity).toLocaleString()} ৳
                                </p>
                              </div>
                            </div>
                            <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-4 bg-black/40 p-1.5 sm:p-2 rounded-xl sm:rounded-2xl border border-white/10">
                              <button 
                                onClick={() => updateQuantity(item.id, item.quantity - 1, item.size)}
                                className="w-6 h-6 sm:w-8 sm:h-8 flex items-center justify-center bg-white/5 rounded-lg sm:rounded-xl hover:bg-white/10 text-white"
                              >
                                <Minus size={12} />
                              </button>
                              <span className="w-4 text-center font-bold text-white text-[10px] sm:text-sm">{item.quantity}</span>
                              <button 
                                onClick={() => updateQuantity(item.id, item.quantity + 1, item.size)}
                                className="w-6 h-6 sm:w-8 sm:h-8 flex items-center justify-center bg-white/5 rounded-lg sm:rounded-xl hover:bg-white/10 text-white"
                              >
                            ...
                            <button 
                              ...
                              className="w-10 h-10 flex items-center justify-center text-white/20 hover:text-red-500 transition-colors"
                            >
                              <Trash2 size={18} />
                            </button>
                          </motion.div>
                        ...
                        <div className="text-center py-20 bg-white/5 rounded-3xl border border-dashed border-white/20">
                          <p className="text-white/30 italic">আপনার কার্ট খালি। শপিং শুরু করুন!</p>
                        </div>
                      )}
                    </AnimatePresence>
                  </div>
                  ...
                  {cart.length > 0 && (
                    <div className="pt-8 border-t border-white/10">
                      ...
                      <div className="flex justify-between items-center text-white/60 mb-6">
                ...
                <div className="w-full lg:w-[450px]">
                  <div className="bg-white/5 p-8 rounded-[3rem] border-2 border-white/10 relative shadow-2xl backdrop-blur-3xl">
                    <form 
                      onSubmit={handleOrderSubmit}
                      className="space-y-4"
                    >
                      <div className="space-y-2 group">
                        <label className="text-[10px] uppercase font-black tracking-widest text-white/60 flex items-center gap-2 mb-2 ml-2">
                        ...
                        <input 
                          ...
                          className="w-full bg-black/60 border border-white/20 rounded-2xl px-6 py-4 text-white placeholder:text-white/30 focus:outline-none focus:border-neon-blue focus:ring-4 focus:ring-neon-blue/10 transition-all font-medium text-sm shadow-inner"
                        />
                      </div>
                      ...
                      <div className="space-y-2 group">
                        ...
                        <textarea 
                          ...
                          className="w-full bg-black/60 border border-white/20 rounded-2xl px-6 py-4 text-white placeholder:text-white/30 focus:outline-none focus:border-neon-blue focus:ring-4 focus:ring-neon-blue/10 transition-all text-sm resize-none shadow-inner"
                        />
                      </div>
                      ...
                      <div className="space-y-2 group">
                        ...
                        <input 
                          ...
                          className="w-full bg-black/60 border border-white/20 rounded-2xl px-6 py-4 text-white placeholder:text-white/30 focus:outline-none focus:border-neon-blue transition-all text-sm shadow-inner"
                        />
                      </div>
                      <div className="pt-4">
                        <button 
                        ...
                        >
                          ...
                          <div className="relative w-full bg-white text-black font-black text-xl py-5 rounded-2xl transition-all active:scale-95 flex items-center justify-center gap-3">
                            <CheckCircle size={22} />
        ...
        {orderSuccessData && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative bg-slate-900 border border-white/10 p-8 sm:p-12 rounded-[3rem] text-center max-w-lg w-full shadow-[0_0_100px_rgba(0,242,255,0.2)]"
            >
              ...
              <h3 className="text-3xl font-black text-white mb-4">অর্ডার সফল হয়েছে!</h3>
              ...
              <div className="bg-white/5 border border-white/5 rounded-2xl p-6 mb-8">
                <p className="text-[10px] uppercase font-black tracking-widest text-white/40 mb-2">পরবর্তী অর্ডারের জন্য অপেক্ষা</p>
                <div className="text-3xl font-black text-white font-display">{formatTime(remainingTime)}</div>
              </div>

              <button 
                onClick={() => setOrderSuccessData(null)}
                className="w-full py-4 bg-neon-blue text-black font-black rounded-xl shadow-lg shadow-neon-blue/20 hover:scale-105 transition-all"
              >
                ঠিক আছে
              </button>
            </motion.div>
